...
class PDP(Utils):
    def plot_pdp(self, m, xs, col, log_x=False, plotsize=None, *args, **kwargs):
    ...
    def plot_multi_pdp(self, m, xs, cols, index, plotsize=None, *args, **kwargs):
        pdp = {
            i: self.fill_list(
                list(
    ...
    def plot_ice(
        ...
        **kwargs,
        ...
        if dep_name:
            fig.update_layout(yaxis_title=self.clean_text(dep_name))
        if plotsize:
            fig.update_layout(
                width=plotsize[0],
                height=plotsize[1],
...
class PDPClassified(PDP):
    def __init__(self, m, xs):
        self.m = m
        self.xs = xs

    def plot_pdp(self, *args, **kwargs):
        return PDP().plot_pdp(self.m, self.xs, *args, **kwargs)

    def plot_multi_pdp(self, *args, **kwargs):
        return PDP().plot_multi_pdp(self.m, self.xs, *args, **kwargs)

    def plot_ice(self, *args, **kwargs):
        return PDP().plot_ice(self.m, self.xs, *args, **kwargs)
