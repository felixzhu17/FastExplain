...
def subtract_time(day, delta, unit="d"):
    before_day = pd.to_datetime(day) - pd.Timedelta(delta, unit)
    return before_day.strftime("%Y-%m-%d")


def chunk_dates(start_date, end_date, freq, return_range=False):
    dates = [
        i.strftime("%Y-%m-%d")
...
def bin_columns(bins, format_numbers=False, dp=2, percentage=False, condense_last=True):
    if format_numbers:
        if percentage:
            binned_columns = [
                f"{lower:,.{dp}%} - {upper:,.{dp}%}"
...
def bin_intervals(
    bins, format_numbers=False, dp=2, percentage=False, condense_last=True
):
    if format_numbers:
        if percentage:
            binned_intervals = [f"{i.left:,.{dp}%} - {i.right:,.{dp}%}" for i in bins]
            if condense_last:
                binned_intervals[-1] = f"{bins[-1].left:,.{dp}%}+"

        else:
            binned_intervals = [f"{i.left:,.{dp}f} - {i.right:,.{dp}f}" for i in bins]
            if condense_last:
