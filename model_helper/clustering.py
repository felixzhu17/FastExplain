...
class Clustering(Utils):
    def feature_correlation(self, xs, plotsize=(1000, 1000)):
    ...
    def get_centroid_distance(self, kmeans):
    ...
    def merge_clusters(self, labels, cluster_0, cluster_1):
        return [cluster_0 if i == cluster_1 else i for i in labels]

    def similar_clusters(self, kmeans, similarity_cutoff):
        similarity = self.get_centroid_distance(kmeans)
...
class ClusteringClassified(Clustering):
    def __init__(self, m, xs):
        self.m = m
        self.xs = xs

    def feature_correlation(self, *args, **kwargs):
        return Clustering().feature_correlation(self.xs, *args, **kwargs)
