...
class Sensitivity:
    def sensitivity_test(
        ...
        if pred_func is None:
            ...
            def pred_func(df):
                return self.predict_column(df, m, m_cols)
    ...
    def predict_column(self, df, m, m_cols):
        return m.predict(df[m_cols])


class SensitivityClassified(Sensitivity):
    def __init__(self, m, xs):
        self.m = m
        self.xs = xs

    def sensitivity_test(self, *args, **kwargs):
        return Sensitivity().sensitivity_test(self.xs, self.m, *args, **kwargs)
