...
class Metrics(Utils):
    def r_mse(self, pred, y):
        return round(math.sqrt(((pred - y) ** 2).mean()), 6)

    def m_rmse(self, m, xs, y):
        return self.r_mse(m.predict(xs), y)

    def plot_roc_curve(self, m, xs, y, plot=True, skip_thresholds=None):
    ...
    def confusion_matrix(
        self, m, xs, y, labels=None, normalize=None, colorscale="Blues"
    ):
        pred_y = m.predict(xs)
